        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-white p-6 shadow-sm sm:rounded-lg border border-gray-100">
                            <div>
                                ...
                                <x-text-input id="title" class="block mt-2 w-full text-lg" type="text" name="title" :value="old('title')" placeholder="Misal: Lomba 17 Agustus 2024" required autofocus />
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>

                            <div class="mt-6">
                                <x-input-label for="description" :value="__('Deskripsi Singkat (Opsional)')" class="font-bold" />
                                <textarea id="description" name="description" rows="5" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" placeholder="Tuliskan sedikit cerita atau keterangan tentang foto ini...">{{ old('description') }}</textarea>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white p-6 shadow-sm sm:rounded-lg border border-gray-100">
                            <h3 class="text-md font-bold text-gray-900 border-b pb-2 mb-4">Upload Foto</h3>
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600 justify-center mt-2">
                                            <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500 px-3 py-1 border border-indigo-200">
                                                ...
                                                <input id="image" name="image" type="file" class="sr-only" accept="image/jpeg, image/png, image/jpg" required>
                                            </label>
                                        </div>
                                        <p class="text-xs text-gray-500 mt-2">Format: PNG, JPG, JPEG (Maks. 3MB)</p>
                                    </div>
                                </div>
